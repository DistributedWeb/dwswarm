...
test('connect two peers directly', async ({ is }) => {
  const swarm1 = dwswarm({ bootstrap: [] })
  const swarm2 = dwswarm({ bootstrap: [] })
  swarm1.listen()
...
test('connect two peers using join (announcing peer and lookup peer)', async ({ is }) => {
  ...
  const peer1 = dwswarm({ bootstrap })
  const peer2 = dwswarm({ bootstrap })
...
test('can multiplex 100 topics over the same connection', async ({ same }) => {
  const { bootstrap, closeDht } = await dhtBootstrap()
  const swarm1 = dwswarm({ bootstrap, maxPeers: 20, queue: { multiplex: true } })
  const swarm2 = dwswarm({ bootstrap, maxPeers: 20, queue: { multiplex: true } })
